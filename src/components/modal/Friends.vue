...
async function searchFriends(){
    ...
    const frnds = await authStore.searchFriends(user.name);
    if(frnds.status === 200){
        console.log(frnds.data);
    }
}


function goPlayerVS(playerId, mode='pvp'){
    router.push({name:'playerVs', params:{type:mode, playerId:playerId}})
}
</script>
    ...
    <div class="w-full h-full flex items-center justify-center absolute top-0 left-0 inset-0 z-99 px-3"
        ...
        <div class="w-[90vw] rounded-lg border-2 border-orange-600 px-3 h-auto text-center relative" :style="`background: ${primary}`">
            ...
            <div class="px-4 w-full mt-5 text-white">
                ...
                <div class="mt-5 w-full rounded-md min-h-65 max-h-80 overflow-y-auto" :style="`background: ${primaryLight}`">
                    <div v-for="(friend, index) of authStore.friends" :key="index" class="w-full flex items-center justify-between p-2 border-b-2 border-gray-300">
                        ...
                        <button class="px-3 py-1 rounded-md bg-green-600 text-white w-[100px] cursor-pointer" @click="goPlayerVS(friend.token)">Challenge</button>
                    </div>
                </div>
            </div>
